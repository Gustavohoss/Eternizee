...
export default function CriadorApp() {
  ...
  const handleNextToGiftType = () => setStep('gift-type');
  const handleNextToCustomizeBackground = () => setStep('customize-background');
      ...
      {step === 'theme-selection' && (
        <div className="relative z-10 container mx-auto px-4 pt-16 md:pt-20 pb-16 max-w-3xl min-h-[calc(100vh-80px)] flex flex-col justify-center">
          ...
          <div className="mt-8 md:mt-10 flex flex-col items-center gap-4">
            <Button onClick={handleNextToGiftType} className="w-full max-w-[220px] md:max-w-[260px] bg-primary hover:bg-primary/90 h-10 md:h-12 rounded-xl font-black text-xs md:text-sm shadow-2xl shadow-primary/20 active:scale-95 transition-all">Próximo</Button>
          </div>
        </div>
      )}
      ...
      {step === 'gift-type' && (
        <div className="relative z-10 container mx-auto px-4 pt-16 md:pt-20 pb-16 max-w-4xl min-h-[calc(100vh-80px)] flex flex-col items-center">
          <div className="w-full max-w-2xl text-center md:text-left">
             <button onClick={handleBack} className="group flex items-center gap-2 text-white/40 hover:text-white transition-colors text-[10px] md:text-[11px] font-bold mb-6 md:mb-8 w-fit mx-auto md:mx-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 w-full max-w-2xl">
            <div onClick={() => setSelectedGiftType('amor')} className={cn("relative group cursor-pointer border rounded-2xl p-5 md:p-6 transition-all duration-300 md:col-span-2", selectedGiftType === 'amor' ? "border-primary bg-primary/5 ring-1 ring-primary/20" : "border-white/10 bg-white/5 hover:border-white/20")}>
            ...
            <div onClick={() => setSelectedGiftType('bestie')} className={cn("group cursor-pointer border rounded-2xl p-5 md:p-6 transition-all duration-300", selectedGiftType === 'bestie' ? "border-primary bg-primary/5 ring-1 ring-primary/20" : "border-white/10 bg-white/5 hover:border-white/20")}>
              <div className="space-y-1">
            ...
            <div onClick={() => setSelectedGiftType('maes')} className={cn("relative group cursor-pointer border rounded-2xl p-5 md:p-6 transition-all duration-300", selectedGiftType === 'maes' ? "border-primary bg-primary/5 ring-1 ring-primary/20" : "border-white/10 bg-white/5 hover:border-white/20")}>
          ...
          <div className="mt-10 md:mt-12 w-full max-w-2xl flex justify-center">
            <Button onClick={handleNextToCustomizeBackground} className="w-full max-w-[220px] md:max-w-[260px] bg-primary hover:bg-primary/90 h-11 md:h-12 rounded-xl font-black text-xs md:text-sm shadow-2xl shadow-primary/20 active:scale-95 transition-all">Próximo</Button>
          </div>
        </div>
      )}
            ...
            {step === 'customize-background' && (
              <div className="space-y-8 md:space-y-10 flex flex-col items-center md:items-start">
                ...
                <div className="hidden lg:flex flex-col sm:flex-row items-center gap-5 pt-10 border-t border-white/5 w-full max-w-md">
                  ...
                  <Button onClick={handleNextToPhotos} className="w-full sm:flex-1 h-12 rounded-xl bg-primary text-white font-black text-sm hover:bg-primary/90 transition-all flex items-center justify-center gap-2">Próxima etapa <ChevronRight className="w-4 h-4" /></Button>
                </div>
              </div>
            )}
            ...
            {step === 'photos' && (
              <div className="space-y-8 md:space-y-10 flex flex-col items-center md:items-start">
                <div className="space-y-3 text-center md:text-left">
                  ...
                  <p className="text-xs md:text-base text-white/40 font-medium max-w-md">Adicione até 8 fotos especiais e personalize a moldura.</p>
                </div>
                <div className="w-full max-md space-y-8">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {uploadedPhotos.map((photo, i) => (
                      <div key={i} className="relative aspect-square rounded-2xl overflow-hidden border border-white/10 group bg-white/5">
                        ...
                        <button onClick={() => removePhoto(i)} className="absolute top-2 right-2 bg-red-500 text-white p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-lg"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    ))}
                    ...
                    {showCard && (
                      <div className="space-y-4 pt-4 border-t border-white/5 animate-in fade-in duration-300">
                        ...
                        <div className="flex items-center gap-4">
                          <Popover><PopoverTrigger asChild><button className="flex items-center gap-3 bg-white/5 border border-white/10 p-2 rounded-xl hover:bg-white/10 transition-all"><div className="w-8 h-8 rounded-lg border border-white/10" style={{ backgroundColor: cardColor }} /><div className="text-left pr-2"><p className="text-[9px] font-black uppercase text-white/30">Cor do Card</p><p className="text-[10px] font-mono font-bold">{cardColor}</p></div></button></PopoverTrigger><PopoverContent className="w-auto p-0 border-none bg-transparent" align="start"><ColorPicker selectedBgColor={cardColor} onChange={setCardColor} /></PopoverContent></Popover>
                          <div className="flex flex-wrap gap-1.5">{['#ffffff', '#fdfdfd', '#f4f4f5', '#111111', '#000000'].map((color) => (<button key={color} onClick={() => setCardColor(color)} className={cn("w-6 h-6 rounded-full border transition-all", cardColor === color ? "border-primary scale-110" : "border-white/10")} style={{ backgroundColor: color }} />))}</div>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="space-y-5"><div className="flex items-center justify-center md:justify-start gap-2"><Layers className="w-5 h-5 text-primary" /><h3 className="text-[10px] md:text-sm font-black uppercase tracking-widest text-white/60">Efeito de Passagem</h3></div><div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div onClick={() => setPhotoEffect('slide')} className={cn("cursor-pointer border rounded-2xl p-4 transition-all duration-300 flex flex-col items-center gap-2 text-center", photoEffect === 'slide' ? "border-primary bg-primary/5 ring-1 ring-primary/20" : "border-white/10 bg-white/5 hover:border-white/20")}><Layout className={cn("w-6 h-6", photoEffect === 'slide' ? "text-primary" : "text-white/40")} /><div><p className="text-[10px] font-black uppercase tracking-wider">Slide Suave</p></div></div>
                    <div onClick={() => setPhotoEffect('coverflow')} className={cn("cursor-pointer border rounded-2xl p-4 transition-all duration-300 flex flex-col items-center gap-2 text-center", photoEffect === 'coverflow' ? "border-primary bg-primary/5 ring-1 ring-primary/20" : "border-white/10 bg-white/5 hover:border-white/20")}><Sparkles className={cn("w-6 h-6", photoEffect === 'coverflow' ? "text-primary" : "text-white/40")} /><div><p className="text-[10px] font-black uppercase tracking-wider">Coverflow 3D</p></div></div>
                    <div onClick={() => setPhotoEffect('cards')} className={cn("cursor-pointer border rounded-2xl p-4 transition-all duration-300 flex flex-col items-center gap-2 text-center", photoEffect === 'cards' ? "border-primary bg-primary/5 ring-1 ring-primary/20" : "border-white/10 bg-white/5 hover:border-white/20")}><Copy className={cn("w-6 h-6", photoEffect === 'cards' ? "text-primary" : "text-white/40")} /><div><p className="text-[10px] font-black uppercase tracking-wider">Cards Pilha</p></div></div>
                  </div></div>
                </div>
                <div className="hidden lg:flex flex-col sm:flex-row items-center gap-5 pt-10 border-t border-white/5 w-full max-md">
                  <Button onClick={handleBack} variant="outline" className="w-full sm:w-auto px-8 h-12 rounded-xl border-white/10 bg-white/5 font-black text-sm hover:bg-white/10 transition-all flex items-center gap-2"><ChevronLeft className="w-4 h-4" /> Voltar</Button>
                  <Button onClick={handleNextToPageTitle} className="w-full sm:flex-1 h-12 rounded-xl bg-primary text-white font-black text-sm hover:bg-primary/90 transition-all flex items-center justify-center gap-2">Próxima etapa <ChevronRight className="w-4 h-4" /></Button>
                </div>
              </div>
            )}
            ...
            {step === 'page-title' && (
              <div className="space-y-8 md:space-y-10 flex flex-col items-center md:items-start">
                ...
                <div className="space-y-8 w-full max-w-md">
                  <div className="space-y-4"><Label className="text-[11px] font-black uppercase tracking-wider text-white/60 text-center md:text-left block">Como vai se chamar a história de vocês?</Label><Input value={pageTitle} onChange={(e) => setPageTitle(e.target.value)} placeholder="Ex: João & Maria ou Nossa Historinha" className="bg-white/5 border-white/10 h-14 md:h-16 rounded-xl text-sm md:text-base font-medium focus:border-primary/50 transition-all shadow-inner" /></div>
                  <div className="space-y-6 bg-white/5 p-6 rounded-2xl border border-white/10">
                    ...
                    <div className="space-y-4 pt-4 border-t border-white/5"><Label className="text-[11px] font-bold text-white/50 uppercase">Cor do Texto</Label><div className="flex items-center gap-4">
                        <Popover><PopoverTrigger asChild><button className="flex items-center gap-3 bg-white/5 border border-white/10 p-2 rounded-xl hover:bg-white/10 transition-all group"><div className="w-10 h-10 rounded-lg shadow-inner border border-white/10" style={{ backgroundColor: titleColor }} /><div className="text-left pr-4"><p className="text-[10px] font-black uppercase text-white/30 group-hover:text-primary transition-colors">Personalizar</p><p className="text-xs font-mono font-bold">{titleColor}</p></div></button></PopoverTrigger><PopoverContent className="w-auto p-0 border-none bg-transparent shadow-none" align="start"><ColorPicker selectedBgColor={titleColor} onChange={(color) => { setTitleColor(color); setUserHasManuallyChangedTitleColor(true); }} /></PopoverContent></Popover>
                        <div className="flex flex-wrap gap-1.5">{['#ffffff', '#e11d48', '#ff4da6', '#7c3aed', '#2563eb', '#111111'].map((color) => (<button key={color} onClick={() => { setTitleColor(color); setUserHasManuallyChangedTitleColor(true); }} className={cn("w-6 h-6 rounded-full border transition-transform active:scale-90", titleColor === color ? "border-white scale-110" : "border-white/10")} style={{ backgroundColor: color }} />))}</div>
                    </div></div>
                  </div>
                </div>
                <div className="hidden lg:flex flex-col sm:flex-row items-center gap-5 pt-10 border-t border-white/5 w-full max-md">
                  <Button onClick={handleBack} variant="outline" className="w-full sm:w-auto px-8 h-12 rounded-xl border-white/10 bg-white/5 font-black text-sm hover:bg-white/10 transition-all flex items-center gap-2"><ChevronLeft className="w-4 h-4" /> Voltar</Button>
                  <Button onClick={handleNextToMessage} className="w-full sm:flex-1 h-12 rounded-xl bg-primary text-white font-black text-sm hover:bg-primary/90 transition-all flex items-center justify-center gap-2">Próxima etapa <ChevronRight className="w-4 h-4" /></Button>
                </div>
              </div>
            )}
              ...
              <div className="space-y-8 md:space-y-10 flex flex-col items-center md:items-start">
                ...
                <div className="space-y-8 w-full max-w-md">
                   ...
                   <div className="space-y-6 bg-white/5 p-6 rounded-2xl border border-white/10">
                    ...
                    <div className="space-y-4"><Label className="text-[11px] font-bold text-white/50 uppercase">Fonte da Mensagem</Label><div className="grid grid-cols-2 gap-2">{FONT_OPTIONS.map((f) => (<button key={f.id} onClick={() => setMessageFont(f.id)} className={cn("px-4 py-3 rounded-xl border text-xs transition-all", messageFont === f.id ? "border-primary bg-primary/10 text-primary font-bold" : "border-white/10 bg-black/20 text-white/40 hover:border-white/20", f.class)}>{f.name}</button>))}</div></div>
            ...
            {step === 'data-location' && (
              <div className="space-y-8 md:space-y-10 flex flex-col items-center md:items-start">
                ...
                <div className="space-y-6 bg-white/5 p-6 rounded-2xl border border-white/10 w-full max-w-md">
                    ...
                    <div className="space-y-4"><Label className="text-[11px] font-bold text-white/50 uppercase">Fonte do Contador</Label><div className="grid grid-cols-2 gap-2">{FONT_OPTIONS.map((f) => (<button key={f.id} onClick={() => setDateFont(f.id)} className={cn("px-4 py-3 rounded-xl border text-xs transition-all", dateFont === f.id ? "border-primary bg-primary/10 text-primary font-bold" : "border-white/10 bg-black/20 text-white/40 hover:border-white/20", f.class)}>{f.name}</button>))}</div></div>
                    <div className="flex items-center justify-between py-2 border-t border-white/5"><div className="flex items-center gap-2"><Bold className="w-4 h-4 text-white/40" /><Label className="text-[11px] font-bold text-white/50 uppercase cursor-pointer" htmlFor="date-bold-toggle">Negrito</Label></div><Switch id="date-bold-toggle" checked={dateIsBold} onCheckedChange={setDateIsBold} /></div>
                    <div className="space-y-4 py-2 border-t border-white/5">
                      <div className="flex items-center justify-between mb-2"><div className="flex items-center gap-2"><Zap className="w-4 h-4 text-white/40" /><Label className="text-[11px] font-bold text-white/50 uppercase cursor-pointer" htmlFor="date-neon-toggle">Efeito Neon</Label></div><Switch id="date-neon-toggle" checked={dateHasNeon} onCheckedChange={setDateHasNeon} /></div>
                      {dateHasNeon && (<div className="space-y-3 pt-2 animate-in fade-in slide-in-from-top-2"><div className="flex items-center justify-between"><Label className="text-[9px] font-black uppercase tracking-wider text-white/40">Força do Neon</Label><span className="text-[10px] font-black text-primary">{dateNeonStrength}</span></div><Slider value={[dateNeonStrength]} onValueChange={(val) => setDateNeonStrength(val[0])} min={2} max={30} step={1} /></div>)}
                    </div>
                    <div className="space-y-4 pt-4 border-t border-white/5"><Label className="text-[11px] font-bold text-white/50 uppercase">Cor da Data</Label><div className="flex items-center gap-4">
                        <Popover><PopoverTrigger asChild><button className="flex items-center gap-3 bg-white/5 border border-white/10 p-2 rounded-xl hover:bg-white/10 transition-all group"><div className="w-10 h-10 rounded-lg shadow-inner border border-white/10" style={{ backgroundColor: dateColor }} /><div className="text-left pr-4"><p className="text-[10px] font-black uppercase text-white/30 group-hover:text-primary transition-colors">Personalizar</p><p className="text-xs font-mono font-bold">{dateColor}</p></div></button></PopoverTrigger><PopoverContent className="w-auto p-0 border-none bg-transparent shadow-none" align="start"><ColorPicker selectedBgColor={dateColor} onChange={(color) => { setDateColor(color); setUserHasManuallyChangedDateColor(true); }} /></PopoverContent></Popover>
                        <div className="flex flex-wrap gap-1.5">{['#ffffff', '#e11d48', '#ff4da6', '#7c3aed', '#2563eb', '#111111'].map((color) => (<button key={color} onClick={() => { setDateColor(color); setUserHasManuallyChangedDateColor(true); }} className={cn("w-6 h-6 rounded-full border transition-transform active:scale-90", dateColor === color ? "border-white scale-110" : "border-white/10")} style={{ backgroundColor: color }} />))}</div>
                    </div></div>
                </div>
                <div className="hidden lg:flex flex-col sm:flex-row items-center gap-5 pt-10 border-t border-white/5 w-full max-md">
                  <Button onClick={handleBack} variant="outline" className="w-full sm:w-auto px-8 h-12 rounded-xl border-white/10 bg-white/5 font-black text-sm hover:bg-white/10 transition-all flex items-center gap-2"><ChevronLeft className="w-4 h-4" /> Voltar</Button>
                  <Button className="w-full sm:flex-1 h-12 rounded-xl bg-primary text-white font-black text-sm hover:bg-primary/90 transition-all flex items-center justify-center gap-2">Finalizar criação <ChevronRight className="w-4 h-4" /></Button>
                </div>
              </div>
            )}
